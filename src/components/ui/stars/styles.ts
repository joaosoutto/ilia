...
export const StarsWrapper = styled(Box, {
  shouldForwardProp: (prop) => !['isMobile', 'top', 'right'].includes(prop as string),
})<StarsWrapperProps>(({ isMobile, top, right }) => ({
  position: "absolute",
...
export const StarsContainer = styled(Box, {
  shouldForwardProp: (prop) => !['size', 'left', 'top', 'opacity', 'animationDuration', 'animationDelay'].includes(prop as string),
})<StarsContainerProps>(({
  size,
  left,
  top,
